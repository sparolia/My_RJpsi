...
void muon_ID_rates_sig_norm_raw(){

  TTree          *fChain;   //!pointer to the analyzed TTree or TChain
  Int_t           fCurrent; //!current Tree number in a TChain
  
  //Opening File
  TFile *f1 = TFile::Open("RootupleBcTo3Mu_tauChannel.root"); //opening the normalization root file for nn output
  ...
  float mu_pt_gen=0,mu_eta_gen=0,mu_phi_gen=0;
  ...
  vector<short> *signalDecayPresent;
  ...
  fChain->SetBranchAddress("signalDecayPresent",&signalDecayPresent,&b_signalDecayPresent);
  ...
  TH2D* hmu_true_mu_pt_eta = new TH2D("hmu_true_mu_pt_eta","True Pt Vs Eta of unpaired muon",60,-3,3,40,0,20);
  ...
  for(int i=0; i<tree->GetEntries();i++){
    ...
    if(signalDecayPresent->at(0)){
      //if((triggerMatchJpsiTk->at(0)) or (triggerMatchDimuon0->at(0))){
      fake_den_full +=1;
    
    mu_pt_gen = gen_mu_p4->Pt();
    mu_eta_gen = gen_mu_p4->Eta();
	...
	//}
      ...
      hmu_true_mu_pt_eta->Fill(mu_eta_gen,mu_pt_gen);
    }
     }
    //}
  }
  ...
  hmu_fake_eff->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_Soft->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_Tight->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_Loose->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_Global->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_PF->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_Tracker->Divide(hmu_true_mu_pt_eta);
  hmu_mu_eff_Medium->Divide(hmu_true_mu_pt_eta);
  ...
  hmu_true_mu_pt_eta->SetLineColor(kRed);
  hmu_true_mu_pt_eta->GetXaxis()->SetTitle("Eta");
  hmu_true_mu_pt_eta->GetYaxis()->SetTitle("Pt");
  hmu_true_mu_pt_eta->Draw("colz");
  c1->SaveAs("True_mu_pt_eta.png");
